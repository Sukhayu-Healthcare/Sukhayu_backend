...
router.post("/genral", verifyToken, async (req:Request, res:Response) => {
    ...
    console.log("Asha ID from token:", ashaID);
...
router.get("/genral", verifyToken, async (req:Request, res:Response) => {
    const ashaID = (req as any).userID; // Extracted from JWT
    console.log("Asha ID to get from token:", ashaID);
    const pg = getPgClient();

    try {
        const query = `
            SELECT * FROM patient_screening
            WHERE asha_id = $1
...
router.post("/tb-first", verifyToken, async (req, res) => {
    ...
    console.log("Asha ID from token for TB screening:", ashaID);
    const pg = getPgClient();
    ...
    try {
        const query = `
            INSERT INTO tb_patients (
                patient_name, age, gender, mobile, address, asha_id, screening_date,
        ...
        const values = [
            patient_name, age, gender, mobile, address, ashaID, screening_date,
            cough_2_weeks, cough_blood, fever_2_weeks, night_sweats,
            weight_loss, chest_pain, household_tb,
            previous_tb, close_contact_tb, hiv_positive, diabetes,
            tobacco_use, alcohol_dependence,
...
router.get("/tb-first", verifyToken, async (req:Request, res:Response) => {
    const ashaID = (req as any).userID; // Extracted from JWT
    console.log("Asha ID to get TB screenings from token:", ashaID);
    const pg = getPgClient();
    ...
    try {
        ...
        console.error("Error fetching TB screenings:", error);
        return res.status(500).json({ error: "Internal Server Error" });
    }
});
    ...
    try {
        ...
        const query = `
            INSERT INTO tb_followups (
                tb_id, visit_date, phase_of_treatment, visit_type,
...
router.get('/anc', verifyToken, async (req: Request, res: Response) => {
    try {
      ...
      const womanId = req.query.woman_id;
  ...
  router.post("/anc-followup", verifyToken, async (req: Request, res: Response) => {
    try {
      const asha_id = (req as any).userId;   // 👈 from verifyToken
        const pg = getPgClient();
  ...
  router.get("/supervisor/data/:tableName/:date", verifyToken, async (req, res) => {
    ...
    try {
        ...
        `;
        const ashaResult = await pg.query(ashaQuery, [supervisorID]);

        if (ashaResult.rows.length === 0) {
            return res.status(404).json({
                message: "No ASHA workers found under this supervisor"
            });
        }

        const ashaIDs = ashaResult.rows.map(r => r.asha_id);

        // STEP 2: Dynamically select records from requested table
        const dataQuery = `
            SELECT *
            FROM ${tableName}
            WHERE asha_id = ANY($1)
